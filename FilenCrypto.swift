...
class FilenCrypto {
    ...
    func streamDecryptData (input: URL, output: URL, key: String, version: Int, index: Int?, shouldClear clear: Bool = true) throws -> URL {
        try autoreleasepool {
            ...
            if version == 1 {
                ...
                guard let inputStream = InputStream(url: newInput) else {
                    throw NSError(domain: "streamDecryptData", code: 9990, userInfo: nil)
                }
                
                let outputHandler = try FileHandle(forWritingTo: output)
                ...
                if !isCBC {
                    ...
                    while inputStream.hasBytesAvailable && bytesReadTotal < bytesToReadTotal {
                        ...
                        try outputHandler.write(contentsOf: unpadded[0..<unpadded.count])
                    }
                    ...
                    while inputStream.hasBytesAvailable {
                        ...
                        if bytesRead > 0 {
                            ...
                            try outputHandler.write(contentsOf: out[0..<Int(outLength)])
                        }
                    }
                    ...
                    if finalOutLength > 0 {
                        try outputHandler.write(contentsOf: out[0..<Int(finalOutLength)])
                    }
